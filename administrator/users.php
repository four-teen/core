...
$pageDescription = 'Authorized non-student user management page for the administrator module.';
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'save_user') {
            ...
            redirect_to('administrator/users.php?edit_user_id=' . $savedUserId);
        }
        ...
        if ($action === 'delete_user') {
            ...
            redirect_to('administrator/users.php');
        }
...
try {
    ...
    $databaseError = is_local_env()
        ? 'Unable to load user management data. ' . $exception->getMessage()
        : 'Unable to load user management data right now. Please try again.';
}
        ...
        <div class="row align-items-center g-4">
          ...
          <div class="col-lg-4">
            <div class="hero-stat-card">
              <span class="hero-stat-label">Access List</span>
              <h4 class="mb-1"><?= h(format_number($managedUserStats['active'])) ?> active users</h4>
              <p class="mb-0"><?= h(format_number($managedUserStats['total'])) ?> authorized records</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="col-lg-5">
    <div class="card h-100">
      ...
      <div class="card-body">
        ...
        <form method="post" action="<?= h(base_url('administrator/users.php')) ?>">
          ...
          <div class="d-flex flex-wrap gap-2">
            ...
            <?php if ($userManagementForm['user_management_id'] !== ''): ?>
              <a href="<?= h(base_url('administrator/users.php')) ?>" class="btn btn-outline-secondary">Clear Form</a>
            <?php endif; ?>
          </div>
        </form>
      </div>
              ...
              <tr>
                ...
                <td class="text-end">
                  <div class="d-inline-flex gap-2">
                    <a
                      href="<?= h(base_url('administrator/users.php?edit_user_id=' . (int) ($managedUser['user_management_id'] ?? 0))) ?>"
                      class="btn btn-outline-primary btn-sm"
                    >
                      Edit
                    </a>
                    <form method="post" action="<?= h(base_url('administrator/users.php')) ?>" onsubmit="return confirm('Remove this user from the access list?');">
                      <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>" />
